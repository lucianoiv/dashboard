...
// const position = fetchCiudad.results.findIndex(index => index.id = idElemento);


function Elegir(event) {
  
  setElegir(false)
  setCiudad(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&minutely_15=rain&current=temperature_2m,is_day,windspeed_10m&hourly=temperature_2m,precipitation_probability,visibility&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max,windspeed_10m_max&timezone=${timeZone}`);
  
  setCalidadAire(`https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${latitude}&longitude=${longitude}&hourly=pm10,uv_index&timezone=${timeZone}`);
  
}

useEffect(()=>{
  fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${text}&count=1&language=es&format=json`)
  .then(response => {
    if (!response.ok) {
  ...
  return (
    ...
    <div className="App">
      ...
      <div>
        <input type="text" placeholder='Elige otra ciudad...' onChange={(event)=>setText(event.target.value)}></input>
        <input type='button' value='Buscar' onClick={()=>setSearch(text)}></input>
        <p>previsualizar</p>
        {submit === ""?
      <>
      ...
      :
        ...
        {fetchCiudad.map(e => <option value={e.name}>{e.country}</option>)}
        </select>
        </>
      }
      </div>
      ...
      <MainWeather elegir={ciudad} nombre={name} aire={calidadAire} latitude={latitude} longitude={longitude} timezone={timeZone} country={country}/>
      </>}
    </div>
